...
def load_training_sample(
    ...
    if not isinstance(X_pool, pd.DataFrame):
        raise ValueError("X_pool must be a pandas DataFrame.")
    if not isinstance(y_labels, pd.DataFrame):
        raise ValueError("y_labels must be a pandas DataFrame.")

    # Decide matching key in y_labels
    if 'diaSourceId' in y_labels.columns and 'diaSourceId' in X_pool.columns:
        key_col = 'diaSourceId'
        pool_key = 'diaSourceId'
    else:
        raise ValueError("y_labels and X_pool are matched on 'diaSourceId'")
    ...
    if not pd.api.types.is_string_dtype(X_pool[pool_key]):
        logger.warning(f"Converting X_pool[{pool_key}] to string but \
                       index mismatch may occur if trailing zeros were lost in \
                       a scientific notation conversion.")
        X_pool[pool_key] = X_pool[pool_key].astype(str)
    ...
    mask = X_pool[pool_key].isin(ysub[key_col].values)
    X_train = X_pool.loc[mask].copy()
    ...
    X_train.index = X_train[pool_key].astype(str)
    ...
    y_train = pd.Series([key_to_target[k] for k in X_train[pool_key].astype(str)],
                        index=X_train.index,
                        name='target',
                        dtype=int)
