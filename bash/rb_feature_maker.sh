...
# wrapper_rb_v1.sh — set envs, find candidates, call python worker 
# TODO: TEST ON LASAIROXDB1
...
for path in "${produced_paths[@]}"; do
  ...
  $PYTHON $CODEBASE/rb_feature_maker.py --input "$path" >> $LOGDIR/lvra_rb_feature_maker_error.log 2>&1
done
